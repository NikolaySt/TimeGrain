...
[GrainConfiguration(nameof(EchoConfigs))]
public class EchoGrain : LaniGrain
{
    private EchoConfigs? _config;

    public override Task<bool> CanExecuteAsync(IHyperArgs args)
    {
        return Task.FromResult(args is IEchoArgs);
    }

    public override async Task ConfigAsync(IGrainConfig? config = null)
    {
        if (config is EchoConfigs value)
            _config = value;
        else
            if (config == null)
            throw new Exception($"The provided configuration ({nameof(EchoConfigs)}) is null");
    ...
    public override async Task<IHyperArgs> ExecuteAsync(IHyperArgs args)
    {
        try
        {
            ...
            {
                EchoInputArgs echoInputArgs => OnExecuteAsync(echoInputArgs),
    ...
    private async Task<IHyperArgs> OnExecuteAsync(EchoInputArgs args)
    {
        args.SetResult($"Echo: {args.Input}");
        return args;
    }
}
